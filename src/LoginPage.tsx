...
const LoginPage = ({ onLoginSuccess }: { onLoginSuccess: (type?: string) => void }) => {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (response.ok) {
                if (isAutoLogin) {
                    // 서버에서 준 유저 데이터 전체를 저장 (id, 닉네임, 토큰 등)
                    localStorage.setItem('artLogUser', JSON.stringify(result.user || { id: form.id }));
                }
                onLoginSuccess();
            } else {
    ...
    // 2️⃣ [아이디 중복 확인] 백엔드 통신
    const checkDuplicateId = async () => {
        if (!form.id) return alert('아이디를 입력해주세요.');

        try {
            const response = await fetch(`${API_BASE_URL}/check-id?id=${form.id}`);
            const result = await response.json();
